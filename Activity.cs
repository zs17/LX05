    ...
    public class Activity
    {
        ...
        public IEnumerable<Model.Activity>GetVerifyPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql += "select count(1) from activity where activityVerify='审核通过'";
        ...
        public IEnumerable<Model.ActivityNo> GetPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by enfTime desc) as num,activity.*from activity)";
        ...
        public int UpdateImg(Model.Activity active)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update activity set activitypicure=@activityPicture where activityid=@activityId";
                return cn.Execute(sql, active);
            }
        }
